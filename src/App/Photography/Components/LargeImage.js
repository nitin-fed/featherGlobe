...
const LargeImage = (props) => {
    ...
    if(props.loadLargeImage)  {
        ...
        if(props.currentId === 1) {
          icons =  (
                <>
                    ...
                    <i className="fa fa-angle-right arrow right" onClick={props.onNextClicked}></i> 
                </>
            )
        } else if(props.currentId === props.totalCount.length){
            icons =  (
                <>
                    <i className="fa fa-angle-left arrow left" onClick={props.onPreviousClicked}></i>
        ...
        return (
            <div className='largeImage' tabIndex='0' onKeyDown={(event) => handleClick(event)}>
                <div className='imgNavigator'>
                    {icons}
                </div>
                <img src={props.largeImageSrc} alt='Home page' />
                <Backdrop show={props.loadLargeImage} clicked={props.onBackdropClicked}  />
            </div>
        )
    } else {
        return null
